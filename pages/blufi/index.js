...
Page({
  data: {
    ...
    devicesListTemp: [],
    devicesList: [],
    isMoreMsg:false, // 是否选中更多信息
    serverType:'tcp',
    serverTypeName:'TCP服务器',
    ip:"",
    port:null,
    serverTypeOptions:[
      {name: 'TCP服务器', value: 'tcp' },
      {name: 'UDP服务器', value: 'udp' },
      {name: 'MQTT服务器', value: 'mqtt' }
    ],
    showServerTypePopup:false
  },
  setValue(key, value) {
    this.setData({
      [key]: value,
  ...
  onChangeMoreMsg:function(event){
    this.setData({
      isMoreMsg: event.detail,
    });
  },
  changeServerType:function(){
    this.setData({
      showServerTypePopup: true,
    });
  },
  onSelectServerType:function(event){
    this.setData({
      serverType: event.detail.value,
      serverTypeName:event.detail.name,
      showServerTypePopup: false,
    });
  },
  onClose:function(){
  ...
  ipChange:function(event){
    ...
    if(value == ''){
      ...
      wx.showToast({ title: 'IP地址仅限输入英文、数字、符、.和-', icon: 'none' })
      this.setData({
        ip: this.data.ip
      })
    }
  ...
  portChange:function(event){
    ...
    if(value == '' || value == null){
      ...
      wx.showToast({ title: '端口号仅限输入数字', icon: 'none' })
      this.setData({
        port: this.data.port
      })
    }
  ...
  _startConfig: function () {
    ...
    if (this.data.isMoreMsg && !this.data.ip) {
      wx.showToast({
        title: 'IP地址不能为空',
        icon: 'none'
      });
    ...
    if (this.data.isMoreMsg && !this.data.port) {
      wx.showToast({
        title: '端口号不能为空',
        icon: 'none'
      });
    ...
    if(this.data.isMoreMsg){
      ...
      const type = (0x13 << 2) | 0x01
      let custom_payload = [type,0x00,sequenceCounet++]
      var temp_custom_payload = []
      for(var i = 0; i < customData.length; i++){
        var utf8 = this.encodeUtf8(customData[i])
        temp_custom_payload.push(...utf8);
      }
      custom_payload.push(temp_custom_payload.length);
      custom_payload = [...custom_payload,...temp_custom_payload]
      var customArray = new Uint8Array(custom_payload);
      this.writeCharacteristicValue(customArray.buffer)
    }
  },
  connectWifi(){
    wx.showLoading({
