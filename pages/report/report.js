...
Page({
	...
	loadSleepReports(startDate, endDate, wifiMac, selectedDate = null) {
		console.log('[report] 加载睡眠报告 - WiFi MAC:', wifiMac, '开始日期:', startDate, '结束日期:', endDate, '选择日期:',
			selectedDate);
		...
		this.deviceManager.getSleepReportData({
			...
			.then(result => {
				...
				// const formattedReports = result.data.data;
	...
	filterReportsByEndDate(reports, selectedDate) {
		...
		const filteredReports = reports.filter(report => {
			...
			console.log('[report] 报告结束时间:', endTime, '结束日期:', endDateStr, '选中日期:', selectedDate,
				'是否匹配:', endDateStr === selectedDate);
	...
	displayReport(report) {
		...
		if (report.left && report.right) {
			...
			if (!leftAbnormal) {
				...
				// 如果都異常，選擇數據更豐富的
				selectedReport = report.left.sleep_report && report.left.sleep_report.length > 0 ? report.left :
					report.right;
			}
		}
	...
	displayReportDetail(reportDetail) {
		...
		// 直接使用API返回的评分等级
		const scoreLevel = reportDetail.scoreEvaluate || this.deviceManager.getSleepScoreLevel(reportDetail
			.sleepScore);
	...
	calculateFontSize(widthPercent) {
		...
		// 線性插值計算字體大小
		return Math.round(minFontSize + (maxFontSize - minFontSize) * (widthPercent - minWidth) / (maxWidth -
			minWidth));
	},
			...
			.then(result => {
				...
				if (result && result.data && result.data.length > 0) {
					...
					if (reportData.left) {
						...
						console.log('sleep_report長度:', reportData.left.sleep_report ? reportData.left
							.sleep_report.length : 0);
					...
					if (reportData.right) {
						...
						console.log('sleep_report長度:', reportData.right.sleep_report ? reportData.right
							.sleep_report.length : 0);
					...
					const selectedSleepReport = this.deviceManager.getValidSleepReport(reportData.left,
						reportData.right);
